...
export default function SupportTicketsNew() {
  ...
  const { data: ticketDetails, refetch: refetchTicketDetails } = useQuery<any>({
    ...
    queryFn: async () => {
      ...
      const resNew =  await apiRequest("GET", `/api/tickets/${selectedTicketId}`);
      return resNew.json()
    },
                ...
                {tickets.map((ticket) => (
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2 mb-3">
                      ...
                      {isAdmin && (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getCategoryColor(ticket.creatorType)}`}>
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      ...
                      {isAdmin && ticket.assignedToName && (
                        <span className="text-purple-600 font-medium">→ {ticket.assignedToName}</span>
                      )}
                    </div>
                  </div>
                ))}
                    ...
                    {!ticketDetails?.messages || ticketDetails.messages.length === 0 ? (
                      ...
                      ticketDetails.messages.map((msg: Message) => (
                        <div
                          key={msg.id}
                          className={`flex ${
                            msg.senderType === user?.role ? "justify-end" : "justify-start"
                          }`}
                        >
                          <div
                            className={`max-w-md ${
                    ...
                    {isAdmin && (
                      <div className="flex items-center gap-2">
                        ...
                        <label htmlFor="internal" className="text-sm text-gray-600 cursor-pointer">
                          Internal note (not visible to {selectedTicket.creatorType})
                        </label>
                      </div>
                    )}
