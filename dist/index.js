...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    automationExecutions = pgTable("automation_executions", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      automationId: varchar("automation_id").notNull().references(() => automations.id, { onDelete: "cascade" }),
      ...
      variables: jsonb("variables").default({}),
      // Runtime variables for the execution
      error: text("error"),
    ...
    automationExecutionLogs = pgTable("automation_execution_logs", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    ...
    insertAutomationSchema = createInsertSchema(automations).omit({ id: true, createdAt: true, updatedAt: true });
    insertAutomationNodeSchema = createInsertSchema(automationNodes).omit({ id: true, createdAt: true, updatedAt: true });
    insertAutomationExecutionSchema = createInsertSchema(automationExecutions).omit({ id: true, startedAt: true });
    insertAutomationExecutionLogSchema = createInsertSchema(automationExecutionLogs).omit({ id: true, executedAt: true });
    ...
    insertUserActivityLogSchema = createInsertSchema(userActivityLogs).omit({ id: true, createdAt: true });
    ...
    automationNodesRelations = relations(automationNodes, ({ one }) => ({
      automation: one(automations, {
        fields: [automationNodes.automationId],
        references: [automations.id]
      })
    }));
    automationExecutionsRelations = relations(automationExecutions, ({ one, many }) => ({
      automation: one(automations, {
    ...
    automationExecutionLogsRelations = relations(automationExecutionLogs, ({ one }) => ({
...
import { eq as eq8, and, desc as desc7 } from "drizzle-orm";
  ...
  "server/repositories/automation.repository.ts"() {
    ...
    AutomationRepository = class {
      ...
      async getActiveAutomationsByTrigger(trigger, channelId) {
        ...
        return await db.select().from(automations).where(and(...conditions));
      }
  ...
  "server/repositories/analytics.repository.ts"() {
    ...
    AnalyticsRepository = class {
      async getAnalyticsByChannel(channelId, days) {
        ...
        }).from(messageQueue).where(
          sql4`${messageQueue.channelId} = ${channelId} AND ${messageQueue.createdAt} >= ${startDate}`
        ).groupBy(sql4`DATE(${messageQueue.createdAt})`).orderBy(sql4`DATE(${messageQueue.createdAt})`);
        return result.map((row) => ({
...
import { eq as eq10, and as and3, isNull as isNull2, desc as desc9, lt, sql as sql5 } from "drizzle-orm";
  ...
  "server/repositories/message-queue.repository.ts"() {
    ...
    MessageQueueRepository = class {
      ...
      async getMessageStats() {
        const result = await db.select({
          messagesSent: sql5`COUNT(CASE WHEN ${messageQueue.status} = 'sent' THEN 1 END)`.mapWith(Number),
          messagesDelivered: sql5`COUNT(CASE WHEN ${messageQueue.status} = 'delivered' THEN 1 END)`.mapWith(Number),
          messagesFailed: sql5`COUNT(CASE WHEN ${messageQueue.status} = 'failed' THEN 1 END)`.mapWith(Number),
          messagesRead: sql5`COUNT(CASE WHEN ${messageQueue.status} = 'read' THEN 1 END)`.mapWith(Number)
        }).from(messageQueue);
      ...
      async getMessageStatsByChannel(channelId) {
        const result = await db.select({
          ...
          messagesFailed: sql5`COUNT(CASE WHEN ${messageQueue.status} = 'failed' THEN 1 END)`.mapWith(Number),
          messagesRead: sql5`COUNT(CASE WHEN ${messageQueue.status} = 'read' THEN 1 END)`.mapWith(Number)
        }).from(messageQueue).where(eq10(messageQueue.channelId, channelId));
        return result[0] || { messagesSent: 0, messagesDelivered: 0, messagesFailed: 0, messagesRead: 0 };
      }
    };
  }
...
init_db();
init_schema();
import { eq as eq13, and as and4, gte as gte3, lte, count, sql as sql6, desc as desc11 } from "drizzle-orm";
...
var getMessageAnalytics = asyncHandler(async (req, res) => {
  ...
  const messageStats = await db.select({
    date: sql6`DATE(${messages.createdAt})`,
    totalSent: count(messages.id),
    delivered: sql6`COUNT(CASE WHEN ${messages.status} = 'delivered' THEN 1 END)`,
    read: sql6`COUNT(CASE WHEN ${messages.status} = 'read' THEN 1 END)`,
    failed: sql6`COUNT(CASE WHEN ${messages.status} = 'failed' THEN 1 END)`,
    pending: sql6`COUNT(CASE WHEN ${messages.status} = 'pending' THEN 1 END)`
  }).from(messages).innerJoin(conversations, eq13(messages.conversationId, conversations.id)).where(and4(...conditions)).groupBy(sql6`DATE(${messages.createdAt})`).orderBy(sql6`DATE(${messages.createdAt})`);
  const overallStats = await db.select({
    totalMessages: count(messages.id),
    totalDelivered: sql6`COUNT(CASE WHEN ${messages.status} = 'delivered' THEN 1 END)`,
    totalRead: sql6`COUNT(CASE WHEN ${messages.status} = 'read' THEN 1 END)`,
    totalFailed: sql6`COUNT(CASE WHEN ${messages.status} = 'failed' THEN 1 END)`,
    totalReplied: sql6`COUNT(CASE WHEN ${messages.fromUser} = true THEN 1 END)`,
    uniqueContacts: sql6`COUNT(DISTINCT ${conversations.contactPhone})`
  }).from(messages).innerJoin(conversations, eq13(messages.conversationId, conversations.id)).where(and4(...conditions));
  const messageTypes = await db.select({
    direction: messages.direction,
    count: count(messages.id)
  }).from(messages).innerJoin(conversations, eq13(messages.conversationId, conversations.id)).where(and4(...conditions)).groupBy(messages.direction);
  const hourlyDistribution = await db.select({
    hour: sql6`EXTRACT(HOUR FROM ${messages.createdAt})`,
    count: count(messages.id)
  }).from(messages).innerJoin(conversations, eq13(messages.conversationId, conversations.id)).where(and4(...conditions)).groupBy(sql6`EXTRACT(HOUR FROM ${messages.createdAt})`).orderBy(sql6`EXTRACT(HOUR FROM ${messages.createdAt})`);
  res.json({
...
var getCampaignAnalytics = asyncHandler(async (req, res) => {
  ...
  const campaignStats = await db.select().from(campaigns).where(conditions.length > 0 ? and4(...conditions) : void 0).orderBy(desc11(campaigns.createdAt));
...
var getCampaignAnalyticsById = asyncHandler(async (req, res) => {
  ...
  const dailyStats = await db.select({
    date: sql6`DATE(${messages.timestamp})`,
    sent: count(messages.id),
    delivered: sql6`COUNT(CASE WHEN ${messages.status} = 'delivered' THEN 1 END)`,
    read: sql6`COUNT(CASE WHEN ${messages.status} = 'read' THEN 1 END)`,
    failed: sql6`COUNT(CASE WHEN ${messages.status} = 'failed' THEN 1 END)`
  }).from(messages).where(eq13(messages.campaignId, campaignId)).groupBy(sql6`DATE(${messages.timestamp})`).orderBy(sql6`DATE(${messages.timestamp})`);
  ...
  const errorAnalysis = await db.select({
    errorCode: sql6`${messages.errorDetails}->>'code'`,
    errorMessage: sql6`${messages.errorDetails}->>'message'`,
    count: count(messages.id)
  }).from(messages).where(and4(
    eq13(messages.campaignId, campaignId),
    eq13(messages.status, "failed")
  )).groupBy(sql6`${messages.errorDetails}->>'code'`, sql6`${messages.errorDetails}->>'message'`).orderBy(desc11(count(messages.id)));
  res.json({
    campaign: campaign[0],
    dailyStats,
    recipientStats,
...
var getCampaignDetails = asyncHandler(async (req, res) => {
  ...
  const messageStats = await db.select({
    ...
    read: sql6`COUNT(CASE WHEN ${messages.status} = 'read' THEN 1 END)`,
    failed: sql6`COUNT(CASE WHEN ${messages.status} = 'failed' THEN 1 END)`
  }).from(messages).where(eq13(messages.campaignId, campaignId)).groupBy(sql6`DATE(${messages.createdAt})`).orderBy(sql6`DATE(${messages.createdAt})`);
  const recipientStats = await db.select({
    status: messages.status,
    count: count(messages.id)
  }).from(messages).where(eq13(messages.campaignId, campaignId)).groupBy(messages.status);
...
async function exportPDF(req, res) {
  ...
  if (type === "campaigns" || type === "all") {
    const campaignStats = await db.select({
      ...
      totalSent: sql6`SUM(${campaigns.sentCount})`,
      totalDelivered: sql6`SUM(${campaigns.deliveredCount})`
...
var createCampaignSchema = z2.object({
  ...
  apiType: z2.enum(["cloud_api", "marketing_messages"]),
...
init_schema();
init_db();
import { eq as eq14, desc as desc12, and as and5, or, like, gte as gte4 } from "drizzle-orm";
var getMessageLogs = asyncHandler(async (req, res) => {
  ...
  if (conditions.length > 0) {
    baseQuery = baseQuery.where(and5(...conditions));
  }
...
import { eq as eq15, desc as desc13, and as and6, sql as sql8, ne } from "drizzle-orm";
...
var updatePasswordSchema = z4.object({
...
router.get("/members", async (req, res) => {
  try {
    ...
    }).from(users).orderBy(desc13(users.createdAt));
    res.json(members);
  } catch (error) {
...
router.get("/members/:id", async (req, res) => {
  try {
    const [member] = await db.select().from(users).where(eq15(users.id, req.params.id));
    if (!member) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const [existingUser] = await db.select().from(users).where(sql8`${users.email} = ${email} OR ${users.username} = ${username}`);
      if (existingUser) {
  ...
  async (req, res) => {
    try {
      ...
      const [member] = await db.update(users).set({
        ...updates,
        updatedAt: /* @__PURE__ */ new Date()
      }).where(eq15(users.id, id)).returning();
      if (!member) {
        return res.status(404).json({ error: "Team member not found" });
      }
      await db.insert(userActivityLogs).values({
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const [user] = await db.select().from(users).where(eq15(users.id, id));
      if (!user) {
...
router.delete("/members/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const [adminCount] = await db.select({ count: sql8`count(*)` }).from(users).where(and6(
      eq15(users.role, "admin"),
      ne(users.id, id)
    ));
    const [userToDelete] = await db.select({ role: users.role }).from(users).where(eq15(users.id, id));
    if (userToDelete?.role === "admin" && adminCount.count === 0) {
      return res.status(400).json({
        error: "Cannot delete the last admin user"
      });
    }
    const [hasAssignments] = await db.select({ count: sql8`count(*)` }).from(conversationAssignments).where(
      and6(
        eq15(conversationAssignments.userId, id),
    ...
    const [deletedUser] = await db.delete(users).where(eq15(users.id, id)).returning();
    if (!deletedUser) {
      return res.status(404).json({ error: "User not found" });
    }
...
router.get("/activity-logs", async (req, res) => {
  try {
    ...
    }).from(userActivityLogs).leftJoin(users, eq15(userActivityLogs.userId, users.id)).orderBy(desc13(userActivityLogs.createdAt)).limit(100);
    res.json(logs);
  } catch (error) {
...
router2.post("/login", validateRequest2(loginSchema), async (req, res) => {
  try {
    ...
    const results = await db.select().from(users).where(eq16(users.username, username));
...
router2.get("/me", async (req, res) => {
  ...
  const [currentUser] = await db.select().from(users).where(eq16(users.id, user.id));
  if (!currentUser) {
    return res.status(404).json({ error: "User not found" });
  }
...
init_db();
init_schema();
import { eq as eq17, and as and7, or as or2, gte as gte5 } from "drizzle-orm";
  ...
  async updatePendingMessageStatuses() {
    try {
      const pendingMessages = await db.select().from(messages).where(
        and7(
          or2(
            eq17(messages.direction, "outbound"),
            eq17(messages.direction, "outgoing")
          ),
      ...
      for (const message of pendingMessages) {
        ...
        if (updates.status && updates.status !== message.status) {
          await db.update(messages).set(updates).where(eq17(messages.id, message.id));
...
var PostgresSessionStore = connectPgSimple(session);
app.use(
  session({
